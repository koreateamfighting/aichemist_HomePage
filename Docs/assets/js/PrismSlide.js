...
var slideshow = (function (window, undefined) {
    ...
    var slides = [
        '/Docs/images/3d_modeling_a_human_head_with_polygons_in_3ds_max.jpg',
    ...
    function setSizeLeftDiv() {
        let prismHeight = $(".prism-slider canvas").css("height");
        $(".r_article").css("height", prismHeight);
    }
    ...
    function addEvents_() {
        navigation.element.addEventListener('click', function (e) {
            ...
            if (isBullet && !instances.slider.isAnimated) {
                ...
                slideAllTo_(index);
                //배경변경
                let selectCount = parseInt(instances.slider.slidesIndex) + 1;
                $(".movePosition3").removeClass("background3 background1 background2 background4");
                $(".movePosition3").addClass("background" + selectCount);

                //글씨 슬라이드
                let className = ".article_" + (index + 1);
                $(".article_sec").hide();
               
                $(className).show('nomal');
